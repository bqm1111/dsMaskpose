...
from maskpose.models import load_filtered_state_dict
...
def preprocess(
    img: np.ndarray, mean: float = 127.5, std: float = 128.0, device="cuda:0"
) -> torch.Tensor:
...
if __name__ == '__main__':
    ...
    torch.onnx.export(model, dummy_input, "maskpose.onnx", input_names="face_input", output_names="maskpose_vector", verbose=True, export_params=True)
